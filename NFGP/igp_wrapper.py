...
def distillation(cfg, net, reload=True):
    ...
    """
    if hasattr(cfg["NFGP"]["models"], "distill_decoder"):
        distill_decoder = cfg["NFGP"]["models"]["distill_decoder"]
    else:
        distill_decoder = cfg["NFGP"]["models"]["net"]
    ...
    if hasattr(cfg["NFGP"], "distill_opt"):
        distill_opt = cfg["NFGP"]["distill_opt"]
    else:
        distill_opt = cfg["NFGP"]["opt"]
    if hasattr(cfg["NFGP"], "distill_lr"):
        lr = float(getattr(cfg["NFGP"], "distill_lr"))
    else:
        lr = None
...
def deformation(cfg, net):
    ...
    """
    param_lst = []
    if hasattr(cfg["NFGP"]["models"], "deform_decoder"):
        deform_decoder = cfg["NFGP"]["models"]["deform_decoder"]
    ...
    if hasattr(cfg["NFGP"], "opt_deform"):
        deform_opt = cfg["NFGP"].opt_deform
    else:
        deform_opt = cfg["NFGP"].opt
    opt_deform_dec, scheduler_deform_dec = get_opt(param_lst, deform_opt)

    out = DeformationWrapper(
        net, getattr(cfg["NFGP"]["models"], "deform_wrapper", argparse.Namespace()),
        d_decoder, None)
...
def correction(cfg, net):
    ...
    a network defined by the configuration [cfg["NFGP"]["models"].correct_decoder]
    :param cfg: Whole configuration file.
    :param net: The neural field.
    :return:
        [out]
    ...
    """
    param_lst = []
    if hasattr(cfg["NFGP"]["models"], "correct_decoder"):
        correct_decoder = cfg["NFGP"]["models"].correct_decoder
    ...
    if hasattr(cfg["NFGP"], "opt_correct"):
        deform_opt = cfg["NFGP"].opt_correct
    else:
        deform_opt = cfg["NFGP"].opt
    opt_deform_dec, scheduler_deform_dec = get_opt(param_lst, deform_opt)

    assert hasattr(cfg["NFGP"]["models"], "deform_wrapper")
    out = DeformationWrapper(
        net, cfg["NFGP"]["models"].deform_wrapper, None, c_decoder)
    return out, opt_deform_dec, scheduler_deform_dec
